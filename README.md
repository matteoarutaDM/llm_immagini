...
Attorno a questa pipeline c'è un livello applicativo completo: **autenticazione utenti** (registrazione con verifica email obbligatoria, login, reset password), distinzione tra **utenti base** e **utenti aziendali** in base al dominio dell'email, **chat persistenti** con SQLite, e **upload di documenti PDF aziendali** isolati per azienda per arricchire la conoscenza RAG delle proprie chat.
...
- `backend/`: API Python FastAPI che riusa la logica del notebook.
  ...
  - `auth.py`, `database.py`: sessioni/token e modello dati SQLite (utenti, aziende, chat, messaggi, documenti, token di verifica/reset).
  - `email_service.py`: invio email di verifica/reset via SMTP (o log del link in sviluppo se SMTP non configurato).
...
- `app/`: interfaccia Next.js (App Router) e proxy verso il backend.
  ...
  - `verify-email/`, `reset-password/`, `privacy/`, `terms/`: pagine dedicate.
