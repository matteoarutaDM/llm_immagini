...
export default function Home() {
  ...
  async function onAuthSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const session = await auth.submitAuth(auth.authMode === "register" ? "register" : "login");
    if (session) await loadWorkspace(session.token);
  }

  async function onSubmitAsk(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    await ask.submit(auth.token, chats.activeChat?.id, chats.appendExchange);
  }

  if (!auth.token) {
    return <AuthPanel auth={auth} onAuthSubmit={onAuthSubmit} />;
  }
