...
export function OcrCard({ identifiers }: { identifiers?: Identifiers }) {
  return (
    <div className="rounded-lg border border-neutral-300 bg-white p-5 shadow-sm dark:border-neutral-700 dark:bg-neutral-900">
      <h3 className="text-lg font-semibold text-neutral-950 dark:text-neutral-50">OCR targhetta</h3>
        ...
        <div className="mt-3 space-y-2 text-sm">
          ...
          {identifiers.visible_text?.length ? (
            identifiers.visible_text.slice(0, 8).map((line, index) => (
              <div key={`${line}-${index}`} className="rounded-md bg-neutral-50 px-3 py-2 font-mono text-xs dark:bg-neutral-800/60 dark:text-neutral-200">
                {line}
              </div>
            ))
          ) : (
...
function IdentifierField({ label, value }: { label: string; value?: string | null }) {
  ...
  return (
    <div className="flex items-start justify-between gap-2 rounded-md bg-neutral-50 px-3 py-2 dark:bg-neutral-800/60">
      <div>
