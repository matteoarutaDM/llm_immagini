...
def test_user_without_company_cannot_upload(client, captured_emails):
    token = signup_login(client, captured_emails, "solo@gmail.com")
    response = _upload(client, token)
    assert response.status_code == 403


def test_company_user_can_upload_valid_pdf(client, captured_emails, monkeypatch):
...
def test_upload_rejects_wrong_content_type(client, captured_emails, monkeypatch):
...
def test_upload_rejects_file_without_pdf_magic_bytes(client, captured_emails, monkeypatch):
    _fake_assistant(monkeypatch)
    token = signup_login(client, captured_emails, "user@digitalmens.it")
...
def test_upload_rejects_oversized_file(client, captured_emails, monkeypatch):
    _fake_assistant(monkeypatch)
...
def test_two_uploads_with_same_original_filename_do_not_collide(client, captured_emails, monkeypatch):
    _fake_assistant(monkeypatch)
    token = signup_login(client, captured_emails, "user@digitalmens.it")
...
def test_delete_document_removes_it_and_invalidates_rag(client, captured_emails, monkeypatch):
    fake = _fake_assistant(monkeypatch)
    token = signup_login(client, captured_emails, "user@digitalmens.it")
...
def test_delete_document_from_another_company_is_not_found(client, captured_emails, monkeypatch):
    _fake_assistant(monkeypatch)
