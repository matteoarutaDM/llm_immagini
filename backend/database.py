...
def init_db() -> None:
    with connect() as connection:
        ...
        # Email verification no longer exists as a gate: this covers both
        # databases from before the "email_verified" column existed (SQLite
        # defaults it to 0 for every pre-existing row above) and any account
        # left "pending" by the old mandatory-verification flow. Idempotent:
        # running this again never touches an already-verified row.
        connection.execute("UPDATE users SET email_verified = 1 WHERE email_verified = 0")

        # Same reasoning for accounts created before the terms/privacy
        # checkbox existed: grandfather them in using their original signup
        # date rather than leaving them with a null ("never consented") value.
        connection.execute("UPDATE users SET terms_accepted_at = created_at WHERE terms_accepted_at IS NULL")

        # The verification-token table is no longer used by any code path.
        connection.execute("DROP TABLE IF EXISTS email_verification_tokens")
